...
随后可以参考之前的文章 [家庭网络连接 | 路由器接入方式 | 旁路由上网 | Router](https://blog.srprolin.top/posts/24-router_1/) 中，关于 **LAN口入网** 的详细配置。

主流路由器现在都有工作模式选择的权利，以我使用的 **锐捷 RG-MA3063(中国移动)** 为例，就可以直接选择 **桥模式**，以避免繁杂的DHCP与IP设置。

![](./2026-03-07_13-55-54.png)

需要注意的是，一般这种桥接模式的路由器，WAN口是连接上游的，同时需要把上游OpenWrt的LAN口作为唯一的DHCP服务器，否则可能会被下游物理路由器AP的底层设置传染。

具体来说就是要在OpenWrt的后台，在 **网络** -> **接口** 处，将DHCP服务器的强制性打开：
![](./2026-07-09-164521.png)
