...
# `<your-server-ip>`为服务器公网 IP 地址，或者也可以使用域名
...
# `<folder>`为文件夹名称，`-R`参数代表递归操作
```

此时用普通用户查看目录中的权限，如果权限不完整可进行调整：
```bash
# 查看当前目录下所有文件、文件夹的类型、权限、链接数、所有者、所属组、大小、修改时间、名称
ll
...
# 为所有者设置读写执行权，为其他用户设置只执行权限
chmod -R u=rwx,o=x <folder>/
```
